...
"""Tests for the Qwen3.8-Flash-Next MTP proposer cache topology."""
...
import vllm.v1.spec_decode.qwen3_8_flash_next as qwen_proposer
...
def _make_proposer_and_config(
...
) -> tuple[Qwen3_8FlashNextMTPProposer, SimpleNamespace]:
    ...
    proposer = Qwen3_8FlashNextMTPProposer.__new__(Qwen3_8FlashNextMTPProposer)
...
def test_speculative_config_selects_qwen_proposer() -> None:
    config = SimpleNamespace(
        ...
        draft_model_config=SimpleNamespace(
            hf_config=SimpleNamespace(model_type="qwen3_8_flash_next_mtp")
        ),
    )

    assert SpeculativeConfig.use_qwen3_8_flash_next_mtp(config)
