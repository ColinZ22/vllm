...
def test_text_checkpoint_mapper_preserves_qwen3_8_flash_next_specific_weights(
    checkpoint_name: str,
    model_name: str,
    shard_id: str | int | tuple[int, ...] | None,
) -> None:
    assert Qwen3_8FlashNextModel.hf_to_vllm_mapper._map_name_with_shard(
        checkpoint_name
    ) == (
        model_name,
        shard_id,
...
def test_vl_checkpoint_mapper_composes_language_and_vision_paths() -> None:
    outer_mapper = Qwen3_8FlashNextForConditionalGeneration.hf_to_vllm_mapper
...
def test_qwen3_8_flash_next_mtp_weight_name_mapping(
    checkpoint_name: str,
    model_name: str | None,
) -> None:
